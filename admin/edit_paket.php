...
if (!isset($_GET['id'])) { header("Location: paket.php"); exit; }
$id = $_GET['id'];

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $stmt = $pdo->prepare("UPDATE pakets SET nama_paket=?, harga=?, deskripsi=? WHERE id=?");
    $stmt->execute([$_POST['nama_paket'], $_POST['harga'], $_POST['deskripsi'], $id]);
    header("Location: paket.php"); exit;
}

$stmt = $pdo->prepare("SELECT * FROM pakets WHERE id=?"); $stmt->execute([$id]);
$paket = $stmt->fetch();
if (!$paket) { header("Location: paket.php"); exit; }
?>
<!DOCTYPE html>
<html lang="id">
<head><title>Edit Paket</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"></head>
<body class="bg-light">
<div class="d-flex">
    ...
    <div class="flex-grow-1 p-4">
        <div class="bg-white p-4 rounded shadow-sm" style="max-width:600px;">
            ...
            <form method="POST">
                <div class="mb-3"><label>Nama Paket</label><input type="text" name="nama_paket" class="form-control" value="<?= htmlspecialchars($paket['nama_paket']) ?>" required></div>
                <div class="mb-3"><label>Harga</label><input type="number" name="harga" class="form-control" value="<?= $paket['harga'] ?>" required></div>
                <div class="mb-3"><label>Deskripsi</label><textarea name="deskripsi" class="form-control" rows="4" required><?= htmlspecialchars($paket['deskripsi']) ?></textarea></div>
                <button type="submit" class="btn btn-danger">Update Paket</button>
